...
function parseRow(r) {
  let platform = (r.platform || "").toLowerCase().trim();
  // Backward compatibility: old exports used `platform = dsp` for both OLV and Display.
  // Infer channel from campaign/ad naming so the dashboard can split them.
  if (platform === "dsp") {
    const camp = String(r.campaign || "").toLowerCase();
    const ad = String(r.ad || "").toLowerCase();
    if (camp.includes("olv") || ad.includes("olv")) platform = "olv";
    else if (camp.includes("display") || ad.includes("display")) platform = "display";
    else platform = "display"; // default so DSP content still appears
  }
  return {
    date: r.date,
    platform,
